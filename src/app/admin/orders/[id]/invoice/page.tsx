...
/**
 * Printable tax invoice. Deliberately black-on-white — this is a paper
 * document, not a screen. GST-ready: sequential invoice number, seller
 * details with GSTIN, itemised amounts. Fragrances are taxed inclusive; the
 * MRP-inclusive figure is what appears, as Legal Metrology requires.
 */
    ...
    <div className="min-h-dvh bg-white p-8 font-sans text-[0.8125rem] leading-relaxed text-neutral-900 print:p-0">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="flex gap-10 border-b border-neutral-300 py-3 text-neutral-600">
          <span>Order: <span className="text-neutral-900">{order.orderNumber}</span></span>
          <span>Placed: <span className="text-neutral-900">{formatDate(order.placedAt ?? order.createdAt)}</span></span>
          <span>Payment: <span className="text-neutral-900">{order.paymentMethod === "COD" ? "Cash on delivery" : "Prepaid (Razorpay)"}</span></span>
        </div>
        ...
        <div className="ml-auto mt-4 w-64 space-y-1.5">
          ...
          <TotalRow label="Delivery">{order.shippingPaise === 0 ? "Free" : formatPaise(order.shippingPaise)}</TotalRow>
